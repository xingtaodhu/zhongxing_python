...
# 读取全部模版到全局变量
def load_template(table_name="mod_template"):
    db = pymysql.connect(host, root, password, database_name,port)
    cs = db.cursor()
    cs.execute(f'select module_name, template from {table_name} WHERE waiting=False AND used=True')
    res = cs.fetchall()
    cs.close()
    db.close()
    global mod_template
...
# 连接数据库
def database_connection(module_name="CMS", table_name="mod_template", is_train="predict", is_used=True):
    # 连接数据库
    db = pymysql.connect(host, root, password, database_name,port=port)
    cs = db.cursor()
    mod_template = {}
    ...
    # 抽取对应分类模板
    if module_name == "ALL":
        mod_template = controller2.extract(cs, db, module_name=module_name, table_name=table_name, is_used=is_used)
    else:
        mod_template[module_name] = controller2.template_extract(cs, db, module_name=module_name, table_name=table_name, is_train=is_train, is_used=is_used)
...
# 删除模块
def delete_center(error_code, module_name="CMS", table_name="mod_template", is_used=None):

    db = pymysql.connect(host, root, password, database_name,port=port)
    cs = db.cursor()

    controller2.template_delete(cs, db, error_code=error_code, module_name=module_name, table_name=table_name, is_used=is_used)
...
def save_center(module_name="CMS", table_name="mod_template", template_name={}, condition=None, is_used=False):
    ...
    controller2.template_save(cs, db, module_name=module_name, table_name=table_name, template_name=template_name, is_backup=is_backup, condition=condition)
...
def update_center(module_name="CMS", table_name="mod_template", template_name={}, error_code=None, condition=None, is_used=True):
    ...
    controller2.template_update(cs, db, module_name=module_name, table_name=table_name, template_name=template_name, error_code=error_code, condition=condition, is_used=is_used)
...
def label_logs_hb(logs, module_name='AAA', template=None):
    ...
    # 存入抽取新分类模型
    controller2.template_save(cs, db, module_name=module_name, table_name="mod_template", template_name=extra_list, is_backup=True)
...
#生日志转data,logs
def raw2dl(logs_data,module_name):
    logs = []
    data = []
    module_log_pos = mod_logpos[module_name.upper()]
    for line in tqdm(logs_data):
        res_l = match_log(line,module_name)
...
###模块日志模版计数
def template_cnt_wi(logs, module_name='aaa'):
    template = mod_template[module_name.upper()]
